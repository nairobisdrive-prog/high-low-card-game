                    ...
                    return (
                      ...
                      >
                        <p className="text-xs font-bold text-gray-400 mb-1.5 uppercase tracking-wide">
                          {sub.is_rando ? '🎲 Rando Cardrissian' : `${submitter?.avatar_emoji ?? '💀'} ${submitter?.display_name ?? 'Unknown'}`}
                        </p>
            ...
            {submissions.length > 0 && (
              <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 mb-4">
                ...
                <div className="space-y-2">
                  {submissions.map((sub) => {
                    const submitter = players.find((p) => p.user_id === sub.user_id);
                    const author = sub.is_rando
                      ? '🎲 Rando Cardrissian'
                      : sub.is_ghost
                        ? `${submitter?.avatar_emoji ?? '💀'} ${submitter?.display_name ?? '?'} 👻 (Ghost)`
                        : `${submitter?.avatar_emoji ?? '💀'} ${submitter?.display_name ?? 'Unknown'}`;
                    return (
